...
class FreeLayoutsEditMode {
  ...
  initEditMode() {
    this.$items.each((index, el) => {
      ...
      $el.find(".ui-resizable-handle").mousedown((e) => {
        let currentDirection = $(e.currentTarget).hasClass("ui-resizable-w")
          ? "w"
          : "e";
  ...
  getPreviousItemInGroup($currentItem, currentIndex) {
    ...
    let $currentGroup = $currentItem.parents(
      `${this.options.groupSelector}:first`
    );
    for (let i = currentIndex - 1; i >= 0; i--) {
      let $previousItem = this.$items.eq(i);
      if (
        $previousItem
          .parents(`${this.options.groupSelector}:first`)
          .is($currentGroup)
      ) {
        return $previousItem;
      }
  ...
  convertAndSaveLayoutItem($el) {
    ...
    let newWidth = parseFloat(($el.outerWidth() / parentWidth) * 100).toFixed(
      2
    );
